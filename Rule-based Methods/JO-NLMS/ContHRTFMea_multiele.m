...
for i = 1 : size(pred_H, 2)
    ...
    e = (y(i) - pred_h'*x');
    p = Eps + est_length*sigw_pow;
    sigx_pow = 1/est_length*norm(x)^2;
    q = p/(est_length*signoise_pow+(est_length+2)*p*sigx_pow);
    u = q*x*e;
    pred_h = pred_h + u';
    Eps = (1-q*sigx_pow)*p;
    sigw_pow = 1/est_length*(norm(u)^2);
    loss = e^2;
...
save('save/JO-NLMS_hrir_p_180_12ch.mat', 'pred_H','-v7.3')
...
saveas(gcf,'plot/JO-True_HRIR0ele.emf')
saveas(gcf,'plot/JO-True_HRIR0ele.fig')
...
xlabel('Time, ms','FontSize',18)
ylabel('Azimuth angle, \circ','FontSize',18)
saveas(gcf,'plot/JO-NLMS_HRIR0ele.emf')
saveas(gcf,'plot/JO-NLMS_HRIR0ele.fig')
...
xlabel('Time, ms','FontSize',18)
ylabel('Azimuth angle, \circ','FontSize',18)
saveas(gcf,'plot/JO-NLMS-ture_HRIR0ele.emf')
saveas(gcf,'plot/JO-NLMS-ture_HRIR0ele.fig')
...
JONLMS_HRTF = abs(fft(pred_H, [], 1));
JONLMS_HRTF = JONLMS_HRTF(1:N/2+1,:,:);
JONLMS_HRTF(2:end-1,:,:) = 2*JONLMS_HRTF(2:end-1,:,:);
JONLMS_HRTF = 20*log10(JONLMS_HRTF);


% analysis of objective performance
freq_ind = find(freq_axis<=20e3 & freq_axis>=50);
ang_ind = 300;
lsd = sum((HRTF_t(freq_ind,:,ang_ind:end) - JONLMS_HRTF(freq_ind,:,ang_ind:end)).^2)/size(HRTF_t,1);
...
figure(5)
im = imagesc(freq_axis/1000,ang_axis,squeeze(JONLMS_HRTF(:,4,:)).'); axis xy
im.AlphaData = .8;
colormap jet
colorbar
hc = colorbar;
...
xlabel('Frequency, kHz','FontSize',18)
ylabel('Azimuth angle, \circ','FontSize',18)
saveas(gcf,'plot/JO-NLMS_HRTF0ele.emf')
saveas(gcf,'plot/JO-NLMS_HRTF0ele.fig')


figure(6)
im = imagesc(freq_axis/1000,ang_axis,squeeze(abs(JONLMS_HRTF(:,4,:)-HRTF_t(:,4,:))).'); axis xy
im.AlphaData = .8;
colormap jet
colorbar
hc = colorbar;
...
xlabel('Frequency, kHz','FontSize',18)
ylabel('Azimuth angle, \circ','FontSize',18)
saveas(gcf,'plot/JO-NLMS-ture_HRTF0ele.emf')
saveas(gcf,'plot/JO-NLMS-ture_HRTF0ele.fig')