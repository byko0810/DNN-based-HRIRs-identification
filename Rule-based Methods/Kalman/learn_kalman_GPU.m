...
%   delta = zeros(os, ss);
  ...
  for ex = 1:N
    y = data{ex};
    T = length(y);
    [beta_t, gamma_t, gamma_c, delta_t, gamma1_t, gamma2_t, x1, V1, loglik_t] = ...
...
%   Q = (gamma2 - A*beta') / Tsum1;
  Q = (gamma2 - A*beta' - beta*A' + A*gamma1*A') / Tsum1;
  if diagQ
    Q = diag(diag(Q));
  end
  if ~ARmode
    R = (alpha - delta' - delta + gamma_C) / Tsum;
    if diagR
      R = diag(diag(R));
    end
  end
...
function [beta, gamma, gamma_c, delta, gamma1, gamma2, x1, V1, loglik] = ...
    Estep(y, A, C, Q, R, initx, initV, ARmode)
%
...
if ARmode
  xsmooth = y;
  Vsmooth = zeros(ss, ss, T, 'gpuArray'); % no uncertainty about the hidden states
  VVsmooth = zeros(ss, ss, T, 'gpuArray');
  loglik = 0;
else
  [xsmooth, Vsmooth, VVsmooth, loglik] = kalman_smoother_GPU(y, A, C, Q, R, initx, initV);
end
...
beta = zeros(ss, ss);
for t=1:T
  delta = delta + y(:,t)*xsmooth(:,t)'*C(t,:)';
%   delta = delta + y(:,t)*xsmooth(:,t)';
  gamma = gamma + xsmooth(:,t)*xsmooth(:,t)' + Vsmooth(:,:,t);
  gamma_c = gamma_c + C(t,:)*(xsmooth(:,t)*xsmooth(:,t)' + Vsmooth(:,:,t))*C(t,:)';
